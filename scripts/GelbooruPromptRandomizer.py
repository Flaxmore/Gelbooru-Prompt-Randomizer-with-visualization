...
async def get_random_tags(include, exclude, current_prompt):
    ...
    if(gel_post == None or gel_post == []):
        # Restituisce il prompt corrente, nessuna immagine e un messaggio di errore
    ...
    # Restituisce i tag, l'URL del file immagine per l'anteprima e l'URL del post per il textbox
...
class GPRScript(scripts.Script):
    ...
    def ui(self, is_img2img):
        with gr.Accordion('Gelbooru Prompt Randomizer', open=False):
            with gr.Column():
                # --- MODIFICA CHIAVE ---
                # Rimosso type="filepath". Ora Gradio gestirà correttamente l'URL.
